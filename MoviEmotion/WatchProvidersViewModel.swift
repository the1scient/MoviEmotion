...
//  MoviEmotion
//
//  Created by MATHEUS DA SILVA MARINI on 30/08/24.
//
...
class WatchProvidersViewModel: ObservableObject {
    @Published var watchProviders: WatchProviderResult?
    ...
    init(movieId: Int) {
        fetchWatchProviders(movieId: movieId)
    }
    
    func fetchWatchProviders(movieId: Int) {
        guard let url = URL(string: "\(baseURL)\(movieId)/watch/providers") else {
            return
        }
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            .map(\.data)
            .decode(type: WatchProvidersResponse.self, decoder: JSONDecoder())
